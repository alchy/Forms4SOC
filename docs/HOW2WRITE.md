...
**Konzistentní.** Jeden pojem = jedno slovo. Přehled termínů pro Forms4SOC:

| Pojem | Používej | Nepoužívej |
|-------|----------|------------|
| JSON soubor incidentu | incident, dokument | záznam, případ |
| JSON soubor šablony | šablona, workbook | template (jako standalone) |
| Analytik v UI | analytik | uživatel, operátor |
| Předdefinované hodnoty | options | hodnoty, možnosti výběru |
