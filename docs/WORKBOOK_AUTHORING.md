...
- Specifická pole v sekci Klasifikace a uzavření
...
| `action_table` | Tabulka akcí Odezva |
| `notification_table` | Tabulka komunikace a notifikací |
| `closure_form` | Klasifikace a uzavření – vždy jako předposlední sekce |
...
posouzeni_trideni (checklist) → odezva → [communication] →
closure → raci
```
...
**Šablonově specifická pole** (např. `attack_type_final`, `affected_service`, `vpn_outage_duration`) přidej **za** `ucl_notification_required` a **před** `root_cause`.
